 ...
 import static com.example.testmlkit.Constants.LD_TIMESTAMP;
 import static com.example.testmlkit.Constants.PREF_CHECKSENS;
 import static com.example.testmlkit.Constants.PREF_CHECKSENS_DEFAULT;
 import static com.example.testmlkit.Constants.T_CATALOG_EXPIRATION;
 import static com.example.testmlkit.R.id.main_menu_refresh;
 import static com.example.testmlkit.R.id.main_menu_set;
 import static com.example.testmlkit.R.id.main_menu_settings;
 import static com.example.testmlkit.R.id.main_menu_sort_author;
 import static com.example.testmlkit.R.id.main_menu_sort_new;
 import static com.example.testmlkit.R.id.main_menu_sort_title;
 import static com.example.testmlkit.Utils.getTimestamp;
 import static com.example.testmlkit.Utils.openLWSetter;
 import static com.example.testmlkit.utils.StorageHelper.backgroundExist;
 import static com.example.testmlkit.utils.StorageHelper.getCacheFolder;
 ...
 import com.example.testmlkit.async.MyAsync;
 import com.example.testmlkit.async.WallpaperDownloader;
 import com.example.testmlkit.components.CatalogAdapter;
 import com.example.testmlkit.components.InfoView;
 import com.example.testmlkit.components.MySwipeRefreshLayout;
 import com.example.testmlkit.data.Catalog;
 import com.example.testmlkit.data.CatalogItem;
 import com.example.testmlkit.services.SyncManager;
 import com.example.testmlkit.utils.InternalData;
 import com.example.testmlkit.utils.StorageHelper;
...
public class MainActivity extends AppCompatActivity implements MyAsync.MyAsyncInterface, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onResume() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            registerReceiver(broadcastReceiver, broadcastFilter, Context.RECEIVER_EXPORTED);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == main_menu_refresh) {// Reload catalog from the server
            loadRemoteContent();
            return true;
        } else if (itemId == main_menu_set) {// Show live-wallpaper preview
            openLWSetter(context);
            return true;
        } else if (itemId == main_menu_settings) {// Start settings
            Intent intent = new Intent(context, SettingsActivity.class);
            startActivity(intent);
            return true;

            // Sort
        } else if (itemId == main_menu_sort_new) {// New
            item.setChecked(true);
            catalog.sort(Catalog.SORT_BY_NEW);
            refreshList();
            return true;
        } else if (itemId == main_menu_sort_title) {// Name
            item.setChecked(true);
            catalog.sort(Catalog.SORT_BY_TITLE);
            refreshList();
            return true;
        } else if (itemId == main_menu_sort_author) {// Author
            item.setChecked(true);
